...
const Screen2Display = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        const res = await axios.get(`${base_URL}/api/consultants`);
  ...
  const updatedTiming = useMemo(() => {
    if (!filterDoctorsData[index]?.scheduleSummary) return "";
    return filterDoctorsData[index].scheduleSummary
      .split(", ")
      .map((item) => {
        const timeRange = item.slice(3);
        const [start, end] = timeRange.split("-");
  ...
  const updatedDays = filterDoctorsData[index]?.scheduleSummary
    ?.split(", ")
    .map((item) => {
  ...
  useEffect(() => {
    setShowDetails(false);
    const detailTimer = setTimeout(() => setShowDetails(true), 3500);
    const nextDoctorTimer = setTimeout(() => {
      setIndex((prev) => {
        if (filterDoctorsData.length <= 1) return 0;
        return (prev + 1) % filterDoctorsData.length;
    ...
    return () => { clearTimeout(detailTimer); clearTimeout(nextDoctorTimer); };
  }, [index, filterDoctorsData]);

  const todayDoctors = useMemo(() => {
    const today = moment().format("ddd");
    return doctorsData?.filter((item) => {
      const days = item?.scheduleSummary?.split(", ").map((it) => it.slice(0, 3));
      return days?.includes(today);
    });
  }, [doctorsData]);
        ...
        {!doctor
          ...
          : <motion.div
            ...
            >
              ...
              {doctor?.imageUrl
                ? <motion.img
                    src={`${base_URL}${doctor.imageUrl}`}
                    alt={doctor?.name}
                    className="rounded-2xl p-2 motion-img shadow-2xl absolute w-[250px] [@media(min-width:2200px)]:w-[290px] [@media(min-width:3200px)_and_(min-height:1000px)]:w-[340px] [@media(min-width:3200px)_and_(min-height:1550px)]:w-[420px] [@media(min-width:4200px)]:w-[540px] h-[300px] [@media(min-width:2200px)]:h-[380px] [@media(min-width:3200px)_and_(min-height:1000px)]:h-[420px] [@media(min-width:3200px)_and_(min-height:1550px)]:h-[520px] [@media(min-width:4200px)]:h-[680px] object-cover border-2 border-[#00b4d8]"
                    initial={{ y: size.h >= 1800 ? -300 : -150, scale: 0.9, x: 0 }}
                    animate={{
                      y: 0,
                      scale: [2.5, 1.8],
              ...
              {showDetails && doctor && (
                ...
                >
                  ...
                  <div className="mt-6 space-y-2 text-[#caf0f8] text-lg [@media(min-width:2200px)]:text-[22px] [@media(min-width:3200px)]:text-[26px] [@media(min-width:4200px)]:text-[32px]">
                    <p><strong className="text-[#90e0ef]">🏥 Room :</strong> {doctor?.roomName || "-"}</p>
                    <p><strong className="text-[#90e0ef]">⏰ Timing :</strong> {updatedTiming}</p>
                    <p><strong className="text-[#90e0ef]">📅 Days :</strong> {updatedDays}</p>
                  </div>
                </motion.div>
              )}
            </motion.div>
      ...
      <div className="absolute bottom-10 flex gap-6">
        {filterDoctorsData.map((_, i) => (
          <button
            key={i}
            onClick={() => setIndex(i)}
            className={`w-5 [@media(min-width:3200px)]:w-7 [@media(min-width:4200px)]:w-10 h-5 [@media(min-width:3200px)]:h-7 [@media(min-width:4200px)]:h-10 rounded-full border-2 transition-all ${
              index === i ? "bg-[#00b4d8] border-[#00b4d8] scale-110" : "border-gray-500 hover:bg-[#0077b6]"
            }`}
          />
        ))}
      </div>
