...
const DoctorTable = () => {
  const [doctorData, setDoctorData] = useState(null);
  const [reRendering, setReRendering] = useState(false);
  const [imageModal, setImageModal] = useState({ open: false, doctor: null });
  const [fileList, setFileList] = useState([]);
  ...
  const handleImageSave = async () => {
    if (!fileList.length) return toast.error("Please select an image");
    setImageLoading(true);
    try {
      const formData = new FormData();
      formData.append("consultant_id", imageModal.doctor?.id);
      formData.append("image", fileList[0]);
      const res = await axios.post(`${base_URL}/api/consultants/image`, formData);
      toast.success(res?.data?.message);
      setImageModal({ open: false, doctor: null });
      setFileList([]);
      setReRendering(prev => !prev);
    } catch (err) {
  ...
  // ---- Image Delete ----
  const handleImageDelete = async (doctor) => {
    setDeleteLoading(doctor?.id);
    try {
      const res = await axios.delete(`${base_URL}/api/consultants/${doctor?.id}/image`);
      toast.success(res?.data?.message);
      setReRendering(prev => !prev);
    } catch (err) {
  ...
  const columns = [
    ...
    { title: "Faculty", dataIndex: "facultyName", key: "faculty", width: 180 },
    // { title: "Degrees", dataIndex: "degrees", key: "degrees", width: 220 },
    // { title: "Mobile", dataIndex: "mobile", key: "mobile", width: 130 },
    { title: "Schedule", dataIndex: "scheduleSummary", key: "schedule", width: 250,
    ...
    },
    {
      ...
      width: 100,
      render: (val) => val
        ? <Image src={`${base_URL}${val}`} alt="doctor" className="h-12 w-12 object-cover rounded" width={48} height={48} />
        : <span className="text-gray-400">-</span>,
    },
    {
      title: "Action",
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <Button
            ...
            icon={<EditOutlined />}
            size="small"
            onClick={() => { setImageModal({ open: true, doctor: row }); setFileList([]); }}
          >
            {row?.imageUrl ? "Edit" : "Add"} Image
          </Button>
          {row?.imageUrl && (
            <Button
              danger
  ...
  return (
    <div>
      ...
      {/* Image Modal */}
      <Modal
        title={`${imageModal.doctor?.imageUrl ? "Update" : "Add"} Image — ${imageModal.doctor?.name}`}
        open={imageModal.open}
        onCancel={() => { setImageModal({ open: false, doctor: null }); setFileList([]); }}
        onOk={handleImageSave}
        okText="Save"
        confirmLoading={imageLoading}
        okButtonProps={{ style: { background: "green" } }}
      >
        {imageModal.doctor?.imageUrl && (
          <div className="mb-4">
            <p className="text-sm text-gray-500 mb-1">Current Image:</p>
            <Image src={`${base_URL}${imageModal.doctor.imageUrl}`} width={100} className="rounded" />
          </div>
        )}
        <Upload
