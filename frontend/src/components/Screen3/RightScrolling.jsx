  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        const newData = rows.map((doc) => {
          const schedule = doc.scheduleSummary?.split(", ").map((item) => {
            const day = item.slice(0, 3);
            const timeRange = item.slice(4, -1);
            const [start, end] = timeRange.split("-");
            return {
            ...
            >
              {visibleDoctors.map((doc, i) => (
                <div
                  key={doc?.id + "-" + i}
                  className="
                    h-[49%] flex m-3 rounded-2xl bg-gradient-to-r
                    from-[#1a3658]/90 to-[#204c79]/90
                    border border-[#00b0ff]/20 shadow-xl overflow-hidden backdrop-blur-sm
                    [@media(min-width:3200px)]:m-5
                    [@media(min-width:4400px)]:m-7
                ...
                >
                  {/* IMAGE */}
                  <div className="w-[40%] h-[100%] overflow-hidden relative p-1">
                    <img
                      src={doc?.imageUrl ? `${base_URL}${doc.imageUrl}` : "/maleDoctor.png"}
                      alt={doc?.name}
                      className="h-full w-full object-fill"
                    />
                    <div className="
