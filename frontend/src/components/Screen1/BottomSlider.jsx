...
const BottomSlider = () => {
  ...
  const UpdateDoctorsData = useMemo(() => {
    return allDoctors.map((doc) => {
      const schedule = doc.scheduleSummary
        ? doc.scheduleSummary.split(", ").map((item) => {
            const day = item.slice(0, 3);
            const timeRange = item.slice(4, -1);
            const [start, end] = timeRange.split("-");
            const start12 = moment(start, "HH:mm").isValid()
              ? moment(start, "HH:mm").format("hh:mm A")
              : null;
            const end12 = end === "00:00"
              ? "12:00 AM"
              : moment(end, "HH:mm").isValid()
                ? moment(end, "HH:mm").format("hh:mm A")
                : null;
            return { day, time: start12 && end12 ? `${start12} - ${end12}` : null };
  ...
  return (
    <div className="relative w-full h-[45vh]
      ...
      {UpdateDoctorsData.length === 0
        ...
        : <div
            className="flex gap-4 h-full transition-transform duration-1000 ease-in-out [@media(min-width:2200px)_and_(min-height:1200px)]:gap-6"
            style={{ transform: `translateX(-${slideIndex * CARD_WIDTH}px)` }}
          >
            {visibleCards.map((doc, i) => (
              <div
                key={`${doc.id}-${i}`}
              ...
              >
                <img
                  src={doc?.imageUrl ? `${base_URL}${doc.imageUrl}` : "/placeholder-doctor.jpg"}
                  alt={doc.name}
                  className="p-1 w-[70px] h-[70px] object-fill
                    [@media(min-width:3200px)]:w-[90px] [@media(min-width:3200px)]:h-[90px]
                    [@media(min-width:3200px)_and_(min-height:2000px)]:w-[100px] [@media(min-width:3200px)_and_(min-height:2000px)]:h-[100px]
                ...
                <div className="flex flex-col
                  ...
                  <div className="text-sm
                    ...
                    {doc.schedule?.length
                      ...
                      : <p className="col-span-2 text-gray-400 italic text-sm">Schedule not available</p>
                    }
                  </div>
