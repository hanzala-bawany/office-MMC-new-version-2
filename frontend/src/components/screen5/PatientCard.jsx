...
const PatientCard = ({ doc }) => {
  ...
  return (
    ...
    <div className="bg-white/95 backdrop-blur-xl rounded-2xl border border-cyan-100 shadow-lg  transition-shadow p-2 flex flex-col h-full">
      <div
        key={patient.id}
        className={`p-6 rounded-xl border-2 transition-all duration-300 flex justify-between flex-1 bg-gradient-to-r from-cyan-100 to-cyan-50 border-yellow-200 shadow-md `}
      >
        <div className="flex flex-col justify-between  space-y-2 text-gray-700">
          <p className="text-lg xl:text-2xl  text-cyan-600 font-semibold">
