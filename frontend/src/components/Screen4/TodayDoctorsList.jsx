...
const TodayDoctorsList = () => {
  ...
  // Sirf aaj ke doctors filter karo
  const udateDoctorsData = useMemo(() => {
    const today = moment().format("ddd"); // "Mon", "Tue" etc
    return allDoctors.filter((doc) => {
      const days = doc.scheduleSummary?.split(", ").map((it) => it.slice(0, 3));
  ...
  useEffect(() => {
    const updateHeight = () => {
      const item = document.querySelector(".doctor-item");
      if (item) setItemHeight(item.clientHeight);
    };
    updateHeight();
    window.addEventListener("resize", updateHeight);
    return () => window.removeEventListener("resize", updateHeight);
